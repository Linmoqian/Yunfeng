# TODO

- [x] 了解项目结构与当前进度
- [x] 调研 pi-web 与 pi agent 的数据共享机制
- [x] 移除 app/sidecar 与 Tauri sidecar 生命周期（后端集中在电脑侧 server + gateway + 移动后端）
- [x] 前端 MVP：会话列表、流式对话、文件工作区、模型切换
- [x] 苹果风格 UI 重构
- [x] 按 prototype skill 重构为三变体 UI（A 命令面板 / B 工程工作台 / C 沉浸对话）
- [x] 工程师选定 UI 变体（已废弃：git 78b8aed 清空重写为液态玻璃单 UI）
- [x] 借鉴 yolo-block 工程经验：新增 RULES.md 项目规则与 MEMORY.md 长期记忆
- [x] 移动端 GatewayClient 直连电脑侧网关（任务/对话/SSE）
- [x] Vitest 覆盖 GatewayClient（REST 请求 / SSE 订阅 / 设置持久化）
- [x] 接入 lin-workflow 工程规范与 project-engineering skill
- [x] 建立 yunfeng-mobile Tauri 2 + Vite + React 工程骨架
- [x] 实现 Marvis 风格移动端首页（对话入口 / 快捷指令 / Agent 协作 / 模式切换）
- [x] 移动端浅色主题对齐 app 设计 token
- [ ] 锁定 UI 主题并沉淀设计规范文档
- [ ] 前端调试信息总线（AGENTS.md 要求）
- [x] 移除 sidecar 打包与 Rust release 接入需求（已废弃）
- [ ] skills 工作流集成（内置 SKILL.md 包 + 管理界面）
- [ ] 端到端 tauri dev 交互验证

## yunfeng-mobile 移动端前端（见 docs/design/yunfeng-mobile-roadmap.md）

- [x] P0-1 网关任务 API 数据层 + types.ts 契约与单测
- [x] P0-2 流式对话（SSE 增量 / thinking / 停止 / 重试）
- [x] P0-3 工具调用生命周期卡片（含失败展开详情）
- [x] P0-4 Markdown / 代码块渲染（基础版）
- [x] P0-5 会话管理（重命名 / 删除 / 归档 / 搜索 / 空态 / 新对话落库）
- [x] P1-6 Agent 实时状态面板（订阅 AgentEvent）
- [x] P1-7 任务拆解视图（子任务进度）
- [ ] P1-8 快捷指令 / 技能真实化（接 registry）
- [ ] P1-9 本地 / 效率模式生效
- [ ] P1-10 授权确认 UI（human-in-the-loop）
- [ ] P2-11 语音输入
- [ ] P2-12 设备协同入口 UI
- [ ] P2-13 人设管理入口
- [ ] P2-14 移动端动效 / 触感打磨
- [ ] P2-15 离线 / 低资源模式提示
- [ ] P2-16 无障碍（字体缩放 / 对比度 / 读屏）
- [x] 引入主应用同款任务看板（Dashi）至移动端标签页

## 看板移动端适配（见 docs/design/yunfeng-mobile-taskboard-mobile.md）

- [x] 设计方向：按 A 液态玻璃统一推进（工程师以「开工」确认）
- [x] TB-P1 看板内导航收敛（隐藏 app-nav、视图切换横向滚动）
- [x] TB-P1 看板列横向滑动吸附 + 卡片触控密度（列宽 84vw、触控目标 ≥44px）
- [x] TB-P1 任务详情移动端间距（全屏页；滑动关闭留后续）
- [x] TB-P2 长按打开操作单（pointer 长按实现；真机手势验证待补）
- [ ] TB-P2 属性/标签/关系选择器改底部弹层
- [ ] TB-P2 列表视图与 Dashboard 移动适配
- [ ] TB-P3 甘特/工作流触控能力评估
- [ ] TB-P3 动效/安全区/真机验证
- [ ] 移动端 Rust cargo check / tauri 启动验证
- [ ] Playwright 移动视口 e2e

## yunfeng-mobile 后端（yunfeng-mobile-backend，见 docs/design/yunfeng-mobile-backend.md）

- [x] 账号模块：配对码 / 设备注册 / token（node:sqlite）
- [x] 移除 sidecar/relay 桥接；WS hub 收敛为心跳 + 远程桌面
- [x] 远程桌面：screencapture JPEG 帧流 + CGEvent 输入注入 helper
- [x] 服务装配与冒烟（YF_MOBILE_READY 协议）
- [x] 调试 CLI（yf-cli）：配对 / 远程桌面帧 / 设备管理
- [x] 移动端设置页：网关地址/token 保存与连通测试
- [x] 移动端配对与远程屏幕浮层（WebSocket 帧查看）
- [ ] 屏幕录制 / 辅助功能权限下的真实帧率与输入验证
- [ ] 移动端浏览器端到端回归（当前以 CLI 协议验证替代）
