...
export default function HomeView({ onOpenSession }: Props) {
  ...
  return (
    <div className="relative min-h-full px-5 pb-8 pt-6">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-64 bg-[radial-gradient(60%_100%_at_50%_0%,rgba(76,201,240,0.16),transparent)]" />
      ...
      <div className="relative">
        ...
        <button
          ...
          className="mt-6 flex w-full items-center gap-3 rounded-2xl border border-edge bg-white/[0.05] px-4 py-4 text-left transition active:scale-[0.99]"
        >
          <span className="grid size-9 shrink-0 place-items-center rounded-xl bg-accent-soft text-accent">
            <Sparkles className="size-[18px]" />
          </span>
          <span className="flex-1 truncate text-[15px] text-ink-dim">输入指令或提问…</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          {quickCommands.map((q) => (
            <button
              ...
              className="rounded-2xl border border-edge bg-white/[0.04] p-4 text-left transition hover:border-edge-strong active:scale-[0.98]"
            >
              <span className="grid size-9 place-items-center rounded-xl bg-white/[0.06] text-accent">
                <q.icon className="size-[18px]" />
              </span>
              <p className="mt-3 text-[15px] font-medium">{q.title}</p>
              <p className="mt-0.5 text-xs text-ink-dim">{q.subtitle}</p>
            </button>
          ))}
        </div>

        <SectionTitle>Agent 协作中</SectionTitle>
        <div className="overflow-hidden rounded-2xl border border-edge bg-white/[0.04]">
          {agents.map((a, i) => (
            <div
              key={a.id}
              className={cn("flex items-center gap-3 px-4 py-3", i > 0 && "border-t border-white/[0.05]")}
            >
        ...
        <div className="space-y-2">
          {sessions.slice(0, 2).map((s) => (
            ...
            >
              <span className="grid size-8 shrink-0 place-items-center rounded-lg bg-white/[0.06] text-ink-dim">
...
function SectionTitle({ children }: { children: ReactNode }) {
  return <h2 className="mb-3 mt-7 text-[13px] font-medium text-ink-dim">{children}</h2>;
}

function ModeSwitch({ mode, onChange }: { mode: RunMode; onChange: (m: RunMode) => void }) {
  return (
    <div className="flex shrink-0 items-center rounded-full border border-edge bg-white/[0.04] p-0.5">
      <ModeButton
        active={mode === "local"}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "bg-accent font-medium text-surface shadow-[0_0_16px_rgba(76,201,240,0.35)]"
          : "text-ink-dim",
      )}
    >
      {icon}
...
function StatusBadge({ status }: { status: AgentStatus }) {
  const map = {
    working: { label: "工作中", dot: "bg-cyan-400 animate-pulse", text: "text-cyan-300" },
    online: { label: "在线", dot: "bg-emerald-400", text: "text-emerald-300" },
    idle: { label: "空闲", dot: "bg-zinc-500", text: "text-zinc-400" },
  } as const;
  const s = map[status];
  return (
