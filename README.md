...
#   外壳自动拉起 server + gateway + mobile-backend + RustDesk；
#   托盘展示各服务状态、分项启停与「复制连接信息」

# 方式二：手动分服务（后端开发时）
cd server && npm install && npm run dev          # 8000
cd gateway && npm install && npm run dev         # 8787，默认 upstream=http://127.0.0.1:8000
cd yunfeng-mobile-backend && npm install && npm run dev   # 8788（Node 24）

# 移动端薄客户端（浏览器预览 / 桌面壳验证）
cd mobile-app && npm install && npm run dev      # Vite 预览
cd mobile-app && npm run tauri dev               # Tauri 壳（含系统通知）

# TUI
cd yunfeng-cli && npm install && npm run cli     # 交互入口；npm run demo 为演示模式
```
...
| 模块 | 命令 |
| --- | --- |
| mobile-app | `npm test`（Vitest 纯逻辑）、`npm run build`（tsc + Vite）、`npm run check:api`（真实 API 联通检测） |
| server | `npm test`（node --test）、`npm run build`（tsc） |
| gateway | `npm test`（node --test） |
| yunfeng-mobile-backend | `npm test`（node --test）、`npm run smoke`（READY/配对/WS 联调） |
| yunfeng-cli | `npm test`（TUI + CLI Vitest）、`npm run smoke` |
| app 外壳 | `cd app/src-tauri && cargo fmt --check && cargo clippy` |
