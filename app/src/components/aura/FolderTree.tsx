  ...
  return (
    <div className="pt-2 space-y-1 border-t border-slate-200/60">
      <div className="px-2 py-1 flex items-center justify-between text-xs font-semibold text-slate-500 uppercase tracking-wider">
        ...
        {hasRoot && (
          <button onClick={onNewFolder} className="hover:text-indigo-600 transition pixel-press" title="新建文件夹">
      ...
      {!hasRoot ? (
        <button
          ...
          className="w-full flex items-center justify-center space-x-2 px-3 py-2 rounded-xl border border-dashed border-slate-300 hover:border-indigo-400 hover:text-indigo-600 text-slate-500 text-xs transition pixel-press"
        >
          <Icons.FolderPlus className="w-3.5 h-3.5" />
          <span>选择项目目录</span>
        </button>
    ...
    return (
      <div>
        <button
          ...
          className="w-full flex items-center space-x-2 px-2.5 py-1.5 rounded-lg hover:bg-slate-200/40 cursor-pointer text-slate-700 font-medium select-none text-left shadow-sm pixel-press"
          style={pad}
        >
          <Icons.ChevronRight
            className={`w-3 h-3 text-slate-500 transition-transform shrink-0 ${node.expanded ? "rotate-90" : ""}`}
          />
          {node.expanded ? (
